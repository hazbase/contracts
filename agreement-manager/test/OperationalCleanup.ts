...
async function buildOfferFixture(manager: any, overrides: Partial<Record<string, unknown>> = {}) {
  ...
  return {
    ...
    offer: {
      ...
      documentHash: overrides.documentHash ?? ethers.keccak256(ethers.toUtf8Bytes('incident-otc-transfer')),
      documentURI: overrides.documentURI ?? 'ipfs://hazbase/ops/incident-otc-offer',
...
describe('AgreementManager operational cleanup coverage', function () {
  it('cancels a delegated offer after incident and prevents future delegated settlement', async function () {
    ...
    await expect(manager.connect(delegatedMarket).acceptOffer(offerId, investorSig)).to.be.revertedWith('not offered');
  });

  it('emits ops ledger for OPS-REC-01 delegated OTC unwind after incident freeze', async function () {
    const [admin, issuer, investor, delegatedMarket] = await ethers.getSigners();
    const token = await ethers.deployContract('MockERC20', ['Stable Token', 'USDS']);
    const manager = await deployAgreementManager(admin.address);
    ...
    expect(await token.balanceOf(issuer.address)).to.equal(1_000n);
    expect(await token.balanceOf(await manager.getAddress())).to.equal(0n);
    ...
    emitOpsLedger({
      ...
      ledger: {
        otc_offer_state: 'cancelled_unwound',
      },
      checks: {
        outstanding_offer_unwound: true,
        no_orphaned_escrow: true,
      },
    });
  });
});
