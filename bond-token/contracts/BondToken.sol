...
interface IERC3475 {
    ...
    /// @notice Burn `amount` from `from`. Typically operator or owner only.
    function burn(
        address from,
        uint256 classId,
        uint256 nonceId,
    ...
    RolesCommonUpgradeable
{
    ...
    /// @notice Constructor disables initializers for the UUPS proxy pattern.
    ...
    /// @dev Calls the Pausable, UUPS, EIP712, ERC2771, and RolesCommon initializers.
    ...
    /// @notice Set the optional whitelist or registry contract.
    /// @param registry Address of the whitelist contract, or zero to disable checks.
    /// @dev Only `MINTER_ROLE` can set this. Transfers between non-zero addresses are checked when set.
    ...
    /// @param to Recipient address.
    /// @dev No checks run if the registry is unset or if this is a mint or burn path.
    ...
    /// @param ok True to allow transfers; false to lock them.
    /// @dev Requires the class to exist and emits `ClassTransferableSet`.
    /// @custom:reverts CLASS_UNKNOWN if class has not been created yet
    ...
    /// @notice Read the current EIP-712 nonce for `owner`.
    ...
    /// @return True if `operator` is approved for all of `owner`.
    ...
    /// @dev This is the direct on-chain approval path; no signature is required.
    function setApprovalForAll(address operator, bool approved) external {
        _operatorApprovals[_msgSender()][operator] = approved;
    }

    /// @notice Create a new snapshot id and finalize all dirty balances and supplies under that id.
    ...
    /// @return Balance recorded at or before snapshot `id`, or 0 if none.
    ...
    /// @param nonceId Nonce id.
    /// @param id Snapshot id (>= 1 recommended).
    /// @return totalSupply recorded at or before snapshot `id`, or 0 if none.
    ...
    /// @param operator Operator address to approve or revoke.
    /// @param approved Approval flag.
    /// @param deadline Signature expiry timestamp (unix seconds).
    /// @param v Recovery param.
    /// @param r Signature r.
    /// @param s Signature s.
    /// @dev Verifies the signed approval, increments the owner's nonce, and updates operator approval state.
    ...
    /// @dev Sets `classTransferable[classId] = true` by default and emits `ClassCreated`.
    /// @custom:reverts CLASS_EXISTS if the class is already initialized
    ...
    /// @param data Array of key/value metadata entries.
    /// @dev Emits `NonceCreated`. The parent class must already exist.
    /// @custom:reverts CLASS_MISSING if the class was not created
    /// @custom:reverts NONCE_EXISTS if the nonce is already initialized
    ...
    /// @notice Mint bond units to `to`.
    /// @param to Recipient address (non-zero).
    /// @param classId Class id (must exist).
    /// @param nonceId Nonce id (must exist).
    /// @param amount Units to mint.
    /// @dev Requires `MINTER_ROLE` and not-paused state. When a whitelist is configured, the recipient is checked.
    /// This updates supply and holder balance, writes snapshot state, and emits `Transfer(address(0), to, ...)`.
    /// @custom:reverts MINT_TO_ZERO if `to == address(0)`
    /// @custom:reverts NONCE_MISSING if nonce metadata was not found
    ...
    /// @dev Allowed if `msg.sender == from` or `isApprovedForAll(from, msg.sender)` is true.
    /// The shared `_transfer` routine enforces pause, transferability, whitelist, and balance checks.
    ...
    /// @param nonceId Nonce id.
    /// @param amount Units to transfer.
    /// @dev Enforces pause, class transferability, whitelist, and non-self-transfer via `_transfer`.
    ...
    /// @param nonceId Nonce id.
    /// @param amount Units to transfer.
    /// @dev Validates class transferability and whitelist, prevents self-transfer, updates balances,
    /// writes snapshot deltas for both parties, and emits `Transfer(from, to, ...)`.
    /// @custom:reverts CLASS_LOCKED if class transfers are disabled
    /// @custom:reverts TRANSFER_TO_ZERO if `to == address(0)`
    ...
    /// @dev Enforces pause and then calls `_burnInternal(_msgSender(), ...)`.
    ...
    /// @notice Burn from `from` (owner, operator, or minter).
    /// @param from Source address.
    /// @param classId Class id.
    /// @param nonceId Nonce id.
    /// @param amount Units to burn.
    /// @dev Allowed if the caller is `from`, an approved operator, or holds `MINTER_ROLE`.
    ...
    /// @notice Internal burn routine with balance, supply, and snapshot updates.
    /// @param from Source address.
    /// @param classId Class id.
    /// @param nonceId Nonce id.
    /// @param amount Units to burn.
    /// @dev Checks balance, updates storage, writes snapshot state, and emits `Redeemed`.
    /// @custom:reverts INSUFF_BAL if `from` balance is below `amount`
    ...
    /// @param classId Class id.
    /// @return Array of key/value metadata.
    ...
    /// @return True if supported, including `IBondTokenMarker` and `IERC3475`.
