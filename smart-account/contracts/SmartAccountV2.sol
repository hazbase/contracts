    ...
    ReentrancyGuardUpgradeable
{
    ...
    /// Unlike v1, validation does not mutate session state so bundler simulation stays deterministic.
    function _validateSignature(
        ...
        if (sigType == SIG_TYPE_OWNER) {
            return IOwnerValidatorV2(ownerValidator).validateUserOpValidationData(
                address(this),
                ownerConfigHash,
                userOpHash,
                sigPayload
        ...
        return _packValidationData(false, uint48(config.validUntil), 0);
    }
